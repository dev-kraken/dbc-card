...
import { z } from "zod";
import { SocialMedia, SocialMediaBackend } from "@/zod/CardSchema";
...
const DragAndDrop = ({
  ...
  function onSubmit(values: z.infer<typeof SocialMedia>) {
    const data = socialMediaInput.map((input, index) => ({
      id: input?.id || input.socialNetworkId,
    ...
    AddUpdateCardSocialMedia(data as z.infer<typeof SocialMediaBackend>).then(
      (r) => {
        if (r?.success) {
        }
  ...
  let dataExist = socialMediaInput.length > 0;
  return (
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              >
                ...
                {cardSocialMedia?.length > 0 ? "Update Media" : "Add Media"}
              </Button>
            </form>
          </Form>
        )}
