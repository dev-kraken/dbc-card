...
import { AllCards, getAvatarUrl } from "@/action/CardCURD";
  ...
  return (
    <PageLayout>
      ...
      <PageLayoutContent className="flex flex-wrap gap-4">
        {allCards &&
          ...
          allCards.map(async (card) => (
            <ListCardsCard
              key={card.id}
              card={card}
              imageUrl={await getAvatarUrl(card.avatarUrl).then((res) =>
                res.toString(),
              )}
            />
          ))}
        {!allCards ||
          (allCards.length < 0 && (
            <section className="w-full border border-dashed border-purple-4/50 rounded-lg flex flex-col items-center justify-center p-6 gap-2">
